...
3. Build & run:
   ...
   ```
4. Open on the AA network: **`http://<server-ip>:9400/rca_console.html`** → **Connect to SQL Server (AA)**.
...
4. Install as an always-on service with [NSSM](https://nssm.cc):
   ```
   nssm install RCAConsole "C:\Path\to\python.exe" "-m uvicorn sql_backend:app --host 0.0.0.0 --port 9400"
   nssm set RCAConsole AppDirectory "C:\Path\to\repo\backend"
   nssm start RCAConsole
   ```
5. Allow TCP 9400 through the server firewall (internal only). Access `http://<server-ip>:9400/rca_console.html`.
...
3. `/etc/systemd/system/rca.service`:
   ...
   ExecStart=/usr/bin/uvicorn sql_backend:app --host 0.0.0.0 --port 9400
   Restart=always
   [Install]
   WantedBy=multi-user.target
   ```
4. `sudo systemctl enable --now rca` · open TCP 9400 internally · access `http://<server-ip>:9400/rca_console.html`.
...
reachability, port 9400, backend + health wait, optional test suites, browser:
